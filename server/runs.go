...
func (r *run) add(e ideacheck.Event) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *run) finish(res *ideacheck.Result, err error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *run) since(from int) ([]ideacheck.Event, *ideacheck.Result, error, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	return append([]ideacheck.Event(nil), r.events[from:]...), r.result, r.err, r.done
}
