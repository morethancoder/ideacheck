...
func result(id, verdict string) *pipeline.Result {
	return &pipeline.Result{ID: id, Status: pipeline.StatusOK, Verdict: verdict, Composite: 0.62, Backend: "mock",
		ConfigHash: "sha256:c", CreatedAt: "2026-09-19T12:00:00Z", Rubric: &pipeline.RubricRef{Name: "business", Hash: "sha256:r"},
		TopRisks: []pipeline.Contribution{{ID: "tarpit", Value: 0.2, Weight: 1.5}}}
}
...
func TestSaveThenFindBySeqIDAndLast(t *testing.T) {
	...
	in := pipeline.Intake{Idea: "  A payroll\n tool  ", Fields: map[string]string{"problem": "fines"}}
	for _, r := range []*pipeline.Result{result("chk_A", "explore"), result("chk_B", "build")} {
