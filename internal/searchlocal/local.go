...
func (l *Local) probe(ctx context.Context) error {
	ctx, cancel := context.WithTimeout(ctx, search.RequestTimeout)
	defer cancel()
...
func limited(err error) (*search.EnginesError, bool) {
	var engines *search.EnginesError
...
func (l *Local) searx() *search.SearX {
	client := l.Client
	if client == nil {
		client = &http.Client{Timeout: search.RequestTimeout}
	}
	return &search.SearX{BaseURL: l.Endpoint, Client: client}
}
